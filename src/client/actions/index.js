...
export const userLogout = () => async (dispatch, getState, api) => {
  if (!getState().auth) return;

  dispatch({ type: GET_CURRENT_USER, payload: false });
...
export const getUsers = socket => async (dispatch, getState, api) => {
  try {
    if (process.env.BROWSER && socket) {
      ...
      socket.on('user-list-response', data => dispatch({ type: GET_USERS, payload: data }));
...
export const getMessages = socket => async (dispatch, getState, api) => {
  try {
    if (process.env.BROWSER && socket) {
      ...
      socket.on('messages-response', data => dispatch({ type: GET_MESSAGES, payload: data }));
